...
Server::Server(const ServConf& conf) : config(conf), listen_fd(-1), isAlive(true) {
...
void Server::init(int epoll_fd) {
    ...
    if (this->listen_fd == -1) {
        std::cerr << "Error: cannot create socket on port " << this->config.port << std::endl;
        this->isAlive = false;
        return;
    }
    fcntl(this->listen_fd, F_SETFL, O_NONBLOCK);
    this->addr.sin_addr.s_addr = inet_addr(this->config.host.c_str());
    ...
    if (bd == -1) {
        if (errno == EADDRINUSE)
            std::cerr << "Error: port " << this->config.port << " already in use" << std::endl;
        else
            std::cerr << "Error: bind on port " << this->config.port << std::endl;
        close(this->listen_fd);
        this->isAlive = false;
        return;
    }
    if (listen(this->listen_fd, SOMAXCONN) == -1) { //? SOMAXCONN --> limite max du noyau
        std::cerr << "Error: socket connection failed on port " << this->config.port << std::endl;
        close(this->listen_fd);
        this->isAlive = false;
        return;
    }
    ...
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, this->listen_fd, &epev) == -1) {
        std::cerr << "Error: epoll_ctl failed on port " << this->config.port << std::endl;
        close(this->listen_fd);
        this->isAlive = false;
        return;
    }
