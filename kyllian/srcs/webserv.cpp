...
//     Server* test_serv = new Server(test);
//     test_serv->init(this->epoll_fd);
//     int listen_fd = test_serv->getListenFd();
//     this->servers[listen_fd] = test_serv;
// }
...
//         if (client->isKeepAlive()) { //? voir avec yasser comment le faire
//             struct epoll_event change_ev;
//             change_ev.data.fd = event_fd;
...
//             if (this->servers.find(event_fd) != this->servers.end()) {
//                 //? on a trouver un server existant donc accepter une nvl connexion
//                 Server* find_server = this->servers[event_fd];
//                 handleNewClient(find_server);
//             }
//             else if(this->clients.find(event_fd) != this->clients.end()) {
...
//             }
//             else
//                 //? ni client ni server, c'est une erreur
//                 std::cerr << "Error: unknow fd find." << std::endl;
//         }
//     }
// }
